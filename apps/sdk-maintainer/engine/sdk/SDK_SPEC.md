...
> - `knowledge`：知识文档，路径解析为 `{app_path}/{path}`，由编译器在 knowledge 注入时自动设置，开发者通常不需要手动声明。
>
> `runtime` 是 `process` 的别名（`type=runtime` 等价于 `type=process`）。
...
加 `restrict_verdict: [verdict1, verdict2]` → 限定目标 step 从该边到达时可输出的 verdict 子集。编译器将各入边的 restrict_verdict 聚合为目标 step 的 verdict_context 字段写入 ROUTER.json（详见 §4.1）。
...
| **carries 推导** | 所有边 | normal confirmed→gate result；normal custom-verdict→gate result + 源角色 process 产出物；backward→源产出+gate+用户反馈（仅 manual 角色）+target自身产出 |
...
| **verdict_context 聚合** | 边的 `restrict_verdict` 子属性 | 编译器从各入边的 restrict_verdict 聚合为目标 step 的 verdict_context 字段，写入 ROUTER.json，供 gate.py 和 router.py 运行时上下文感知 verdict enum 过滤 |
...
> **`verdict_context`（step 级，可选）**：格式为 `{source_step: [allowed_verdicts]}`，记录从不同来源 step 到达本 step 时允许输出的 verdict 子集。该字段由编译器从入边的 `restrict_verdict` 聚合生成。gate.py 和 router.py 运行时读取此字段，结合 STATE.json step_status 中的 `from_steps` 进行动态 verdict enum 过滤（详见 §5.6）。
...
| `workspace_template.init_files` | workspace 初始化时创建的文件映射（字典结构，key 为文件路径） |
...
#### verdict_context 上下文感知过滤（与 edge_counts 过滤正交叠加）

除上述 edge_counts 过滤外，router.py 和 gate.py 还实现了基于 `verdict_context` 的上下文感知过滤：

```
ROUTER.json: step "D" 有 verdict_context = {"A": ["confirmed"], "B": ["confirmed", "challenged"]}
STATE.json: step_status["D"].from_steps = ["A"]

→ dispatch.schema_constraints.verdict_enum = verdict_context["A"] = ["confirmed"]
→ 角色从 A 到达时只能输出 confirmed，不能输出 challenged
```

**机制说明**：
- router.py 组装 dispatch 时，若目标 step 有 verdict_context 且当前 from_steps 匹配某 source_step，则将 schema_constraints.verdict_enum 进一步限定为 verdict_context[source_step] 的子集。
- gate.py 校验时同样读取 STATE.json step_status 中的 from_steps 进行 enum 过滤，确保角色在特定来源路径下只能输出允许的 verdict 值。
- 此过滤与 §5.6 的 edge_counts 过滤**正交叠加**——两者同时生效，取交集。
...
| `knowledge` | `{app_path}/{path}`（解析到应用包目录而非 workspace 目录） |
...
{
  "schema_version": "4.0",
  "workspace_id": "当前项目目录名",
  "step_status": {
    "步骤ID": {"status": "executing", "role": "角色名", "dispatch_id": "ckpt_xxx", "started_at": "ISO时间", "from_steps": ["来源步骤ID"]}
  },
  "finished": {
...
| 字段 | 说明 |
|---|---|
| `step_status` | 正在执行或等待确认的步骤。executing = 正在执行；awaiting_confirmation = 等用户确认。每个条目可选包含 `from_steps` 字段（数组），记录 dispatch 来源 step 列表，供 verdict_context 运行时过滤使用 |
...
```bash
python3 engine/scripts/step.py --submit --step 步骤ID --outputs '[{"name":"x","path":"y"}]' --verdict '<verdict值>' --workspace-id WS_ID [--dispatch-id <dispatch_id>]
```
...
```bash
python3 engine/scripts/step.py --decide --decisions '[{"step":"步骤ID","decision":"confirmed|fail"}]' --workspace-id WS_ID
```
...
| Schema 校验 | 有 schema.json → 校验 required + type + enum。**上下文感知**：若 ROUTER.json 中该 step 有 verdict_context 且 STATE.json step_status 中记录了 from_steps，则用 verdict_context[from_steps] 的子集替换 schema 中的全量 verdict enum 进行校验 |
