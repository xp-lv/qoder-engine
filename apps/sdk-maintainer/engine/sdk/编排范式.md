...
| `knowledge` | 否 | `{app_path}/{path}` | 知识文档，解析到应用包目录。由编译器在 knowledge 注入时自动设置 type='knowledge'，开发者通常不需要手动声明 |
...
### 4.4 restrict_verdict 边级属性

在边上声明 `restrict_verdict` 子属性，限定目标角色从该边到达时可输出的 verdict 子集。编译器将各入边的 restrict_verdict 聚合为目标 step 的 verdict_context 字段写入 ROUTER.json。

```yaml
edges:
  - 需求红队 → 架构执行者 when: result.verdict == "confirmed"
    restrict_verdict: [confirmed, challenged]
```

上例中，架构执行者从需求红队的 confirmed 边到达时，只能输出 confirmed 或 challenged 两种 verdict。编译器将此信息聚合为 ROUTER.json 中架构执行者 step 的 `verdict_context` 字段，供 gate.py 和 router.py 运行时做上下文感知 verdict enum 过滤。

> restrict_verdict 是边级属性，写在边定义下方缩进。多条入边的 restrict_verdict 会聚合为目标 step 的 verdict_context，每条入边贡献一个 `{source_step: [allowed_verdicts]}` 条目。
