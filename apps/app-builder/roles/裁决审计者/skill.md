...
## 执行步骤
1. 读取 dispatch 注入的输入文件（审阅报告[可选] + 需求文档 + app.yaml + 可选的对抗分析报告/压力测试报告）
2. 四维审计：证据质量 / severity 合理性 / 假阴性检测 / 跨轮次一致性
3. 三路径 verdict 路由：
   - 综合裁决者路径：confirmed → 知识管理者 / loop → 架构师 / conditional_pass → 架构师 / requirement_defect → 需求接收者
   - 需求红队路径：req_challenge_upheld → 需求接收者 / req_challenge_overturned → 架构师
   - 架构红队路径：arch_challenge_upheld → 架构师 / arch_challenge_overturned → 综合裁决者

## 判定原则
- 无法判定时默认 upheld（保守策略），携带 needs_human_review 标注
...
- `req_challenge_overturned`：需求红队假阳性翻转 → 架构师
- `arch_challenge_upheld`：架构红队缺陷成立 → 架构师
- `arch_challenge_overturned`：架构红队假阳性翻转 → 综合裁决者
