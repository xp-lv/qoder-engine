...
def main():
                ...
                if step_status:
                    ...
                    if len(steps_to_rollback) == 1:
                        ...
                        for s in steps_to_rollback:
                            rollback_cmd = ["python3", "engine/scripts/set_state.py", "--action", "rollback", "--step", s, "--state-path", args.state_path]
                            run_result = subprocess.run(rollback_cmd, capture_output=True, text=True, timeout=10)
    ...
    cmd = ["python3", "engine/scripts/set_state.py", "--action", mapping["action"], "--step", step, "--state-path", args.state_path]
